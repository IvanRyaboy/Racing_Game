...
public class UpgradedCarController : MonoBehaviour{
    ...
    void runEngine(){
        ...
        if(!engineLerp){
            engineRPM = Mathf.Lerp(engineRPM,1000f + Mathf.Abs(wheelsRPM) *  finalDrive *  (gears[gearNum]) , (EngineSmoothTime * 10) * Time.deltaTime * 1000);
            normalizedRPM = engineRPM / maxRPM;
            totalPower = enginePower.Evaluate(normalizedRPM) * (gears[gearNum] * finalDrive ) * acceleration  ;
        }
    ...
    void runCar(){
        ...
        wheelColliders[2].brakeTorque = wheelColliders[3].brakeTorque = (IsHandbrake())? Mathf.Infinity : 0f;

        rb.angularDrag = (KPH > 100)? KPH / 100 : 0;
        rb.drag = dragAmount + (KPH / 40000) ;
    ...
    void steerVehicle(){
        ...
        horizontal = Mathf.Lerp(horizontal , Input.GetAxis("Horizontal") , (Input.GetAxis("Horizontal") != 0) ? 2 * Time.deltaTime * 1000 : 3 * 2 * Time.deltaTime);
    ...
    void lerpEngine(){
        if(engineLerp){
            totalPower = 0;
            engineRPM = Mathf.Lerp(engineRPM,engineLerpValue,20 * Time.deltaTime * 1000 );
    ...
    void friction(){
        ...
        for (int i = 0; i < wheelColliders.Length ; i++){
            if(wheelColliders[i].GetGroundHit(out hit) && i >= 2 ){
                ...
                forwardFriction.stiffness = (IsHandbrake())?  .55f : ForwardStifness; 
                wheelColliders[i].forwardFriction = forwardFriction;

                sidewaysFriction = wheelColliders[i].sidewaysFriction;
                sidewaysFriction.stiffness = (IsHandbrake())? .55f : SidewaysStifness;
